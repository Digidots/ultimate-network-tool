...
class VLANProber:
    ...
    def _probe_loop(self):
        ...
        try:
            ...
            # Count how many VLANs from our list were detected via DHCP
            dhcp_detected_count = sum(1 for vlan in vlans_to_probe
                                     if vlan in self.discovered_vlans and
                                     self.discovered_vlans[vlan].status == 'Active')

            if dhcp_detected_count >= len(vlans_to_probe):
                # All VLANs detected via DHCP, skip passive listening
                self.logger.info(f"All {len(vlans_to_probe)} VLANs detected via DHCP, skipping passive listening")
                self.running = False  # Stop passive listener
            else:
                # Some VLANs not detected, wait for passive detection
                missing_count = len(vlans_to_probe) - dhcp_detected_count
                self.logger.info(f"Waiting for passive VLAN detection to complete (60 seconds) - listening for {missing_count} non-DHCP VLANs...")
